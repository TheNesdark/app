...
/* JADX INFO: loaded from: GenomaP.jar:Facturacion/Furips_PB_Glosa.class */
public class Furips_PB_Glosa {
    ...
    public Furips_PB_Glosa(String xidPersonaEvento, String xidEvento, String xidIngreso, String xNRadicacionG, String xnValorG, String xTraslado, String xnValorT) {
    ...
    public JasperReportBuilder build() {
        ...
        JasperPdfExporterBuilder pdfExporter = DynamicReports.export.pdfExporter(this.xmt.getRutaRep() + "furipsPB.pdf");
        ...
        try {
            ...
            } else if (xrsAtencionI.getString("Id_TipoAtencion").equals("3") || xrsAtencionI.getString("Id_TipoAtencion").equals("2")) {
                this.xApellido1 = xrsAtencionE.getString("Apellido1");
                this.xApellido2 = xrsAtencionE.getString("Apellido2");
                this.xNombre1 = xrsAtencionE.getString("Nombre1");
                this.xNombre2 = xrsAtencionE.getString("Nombre2");
            ...
            ComponentBuilder componentBuilderNewRow3 = DynamicReports.cmp.horizontalList().add(new ComponentBuilder[]{label("Fecha de ingreso", 9), emptyCell(1), textCell(xrsAtencionI.getString("FechaIngreso"), 8), emptyCell(2), label("a las", 3), emptyCell(1), textCell(xrsAtencionI.getString("HoraIngreso"), 4), emptyCell(2)}).add(new ComponentBuilder[]{label("Fecha de Egreso", 9), emptyCell(1), textCell(this.xFechaEgreso, 8), emptyCell(2), label("a las", 3), emptyCell(1), textCell(this.xHoraEgreso, 4)}).newRow(2).add(new ComponentBuilder[]{label("Código Diagnóstico principal de Ingreso", 16), textCell(this.xDxPIngreso, 4), emptyCell(4), label("Código Diagnóstico principal de Egreso", 18), textCell(this.xDxEgreso, 4)}).newRow(3).add(new ComponentBuilder[]{label("Otro Código Diagnóstico de ingreso", 16), textCell(xrsAtencionI.getString("Codigo_DxR1"), 4), emptyCell(4), label("Otro código Diagnóstico principal de Egreso", 18), textCell(this.xDxEgreso2, 4)}).newRow(3).add(new ComponentBuilder[]{label("Otro Código Diagnóstico de ingreso", 16), textCell(xrsAtencionI.getString("Codigo_DxR2"), 4), emptyCell(4), label("Otro código Diagnóstico principal de Egreso", 18), textCell(this.xDxEgreso3, 4)}).newRow(3).add(new ComponentBuilder[]{labelP(this.xApellido1, 25), emptyCell(9)}).add(new ComponentBuilder[]{labelP(this.xApellido2, 25)}).newRow(2).add(new ComponentBuilder[]{emptyCell(4), label("1er Apellido del Médico o Profesional tratante", 18)}).add(new ComponentBuilder[]{emptyCell(15), label("2do Apellido del Médico o Profesional tratante", 18)}).newRow(2).add(new ComponentBuilder[]{labelP(this.xNombre1, 25), emptyCell(9)}).add(new ComponentBuilder[]{labelP(this.xNombre2, 25)}).newRow(2).add(new ComponentBuilder[]{emptyCell(4), label("1er Nombre del Médico o Profesional tratante", 18)}).add(new ComponentBuilder[]{emptyCell(15), label("2do Nombre del Médico o Profesional tratante", 18)}).newRow(2).add(new ComponentBuilder[]{TipoDocumentoAtencion(this.xTipoDoc), emptyCell(3)}).add(new ComponentBuilder[]{emptyCell(11), label("No. Documento", 9), textCell(this.xNoDocumento, 16)}).newRow(3).add(new ComponentBuilder[]{emptyCell(26), label("Número de Registro Médico", 17), textCell(this.xRegMedico, 16)}).newRow(3);
            String xtotal = "0";
            ...
            if (Double.parseDouble(this.xTotalEPS) - Double.parseDouble(this.xTotalTraslado) > 0.0d) {
                if (xrsaccidente.getInt("EstadoSeg") == 1) {
                    ...
                    xtotal = String.valueOf(this.xmt.formatoMoneda1.format(Double.parseDouble(this.xTotalEPS) - Double.parseDouble(this.xTotalTraslado)));
                    String.valueOf(this.xmt.formatoMoneda1.format(Double.parseDouble(this.xTotalEPS) - Double.parseDouble(this.xTotalTraslado)));
                }
            }
            if (Double.parseDouble(this.xTotalTraslado) > 0.0d) {
                if (xrsaccidente.getInt("EstadoSeg") == 1) {
                    ...
                    String.valueOf(this.xmt.formatoMoneda1.format(Double.parseDouble(this.xTotalTraslado)));
                    xtotaltrasporte = String.valueOf(this.xmt.formatoMoneda1.format(Double.parseDouble(this.xTotalTraslado)));
                }
                if (!this.xNRadicacionG.equals("") && this.xTraslado.equals("1")) {
                    String str = this.xnValorG;
                }
            }
            report.setTemplate(Templates_1.reportTemplate).setPageFormat(612, 1030, PageOrientation.PORTRAIT).setTextStyle(this.textStyle).title(new ComponentBuilder[]{DynamicReports.cmp.image(this.xmt.getRutaRep() + "Encabezado_Furips.png").setFixedDimension(592, 60), DynamicReports.cmp.text("VI. DATOS DEL CONDUCTOR DEL VEHÍCULO INVOLUCRADO EN EL ACCIDENTE DE TRANSITO").setStyle(styleEncabezados), DynamicReports.cmp.verticalGap(3), componentBuilderNewRow, DynamicReports.cmp.verticalGap(3), DynamicReports.cmp.text("VII. DATOS DE REMISION").setStyle(styleEncabezados), DynamicReports.cmp.verticalGap(3), this.Remision, DynamicReports.cmp.verticalGap(3), DynamicReports.cmp.text("VIII. AMPARO DE TRANSPORTE Y MOVILIZACION DE LA VICTIMA").setStyle(styleEncabezados), DynamicReports.cmp.verticalGap(3), componentBuilderNewRow2, DynamicReports.cmp.verticalGap(3), DynamicReports.cmp.text("IX. CERTIFICACION DE LA ATENCIÓN MEDICA DELA VICTIMA COMO PRUEBA DEL ACCIDENTE O EVENTO").setStyle(styleEncabezados), DynamicReports.cmp.verticalGap(3), componentBuilderNewRow3, DynamicReports.cmp.verticalGap(3), DynamicReports.cmp.text("X. AMPAROS QUE RECLAMA").setStyle(styleEncabezados), DynamicReports.cmp.verticalGap(12), DynamicReports.cmp.horizontalList().add(new ComponentBuilder[]{emptyCell(30), DynamicReports.cmp.text("VALOR TOTAL FACTURADO").setStyle(styleCuadroBlanco2), DynamicReports.cmp.text("VALOR RECLAMADO AL FOSYGA").setStyle(styleCuadroBlanco2)}).newRow().add(new ComponentBuilder[]{labelP("GASTOS MEDICO QUIRURGICOS", 30), DynamicReports.cmp.text(xtotal).setStyle(styleCuadroBlanco2), DynamicReports.cmp.text(this.xnValorG).setStyle(styleCuadroBlanco2)}).newRow().add(new ComponentBuilder[]{labelP("GASTOS DE TRANSPORTE Y MOVILIZACION DE LA VICTIMA", 30), DynamicReports.cmp.text(xtotaltrasporte).setStyle(styleCuadroBlanco2), DynamicReports.cmp.text(this.xnValorT).setStyle(styleCuadroBlanco2)}).newRow(5).add(new ComponentBuilder[]{DynamicReports.cmp.text("El total facturado y reclamado descrito en este numeral se debe detallar y hacer descripcion de las actividades, procedimientos, medicamentos, insumos, suministros y materiales, dentro del anexo técnico numero 2.")}), DynamicReports.cmp.verticalGap(3), DynamicReports.cmp.text("XI. DECLARACION DE LA INSTITUCION PRESTADORA DE SERVICIOS DE SALUD.").setStyle(styleEncabezados), DynamicReports.cmp.verticalGap(6), DynamicReports.cmp.horizontalList().add(new ComponentBuilder[]{DynamicReports.cmp.text("Como representante legal o Gerente de la Institución Prestadora de Servicios de Salud, declaró bajo la gravedad de juramento que toda la información contenida en este formulario es cierta y podrá se verificada  por la Dirección General de Financiamiento del Ministerio de la Protección Social , por el Administrador Fiduciario del Fondo de Solidaridad y Garantía Fosyga, por la Superintendencia Nacional de Salud o la  Contraloría General de la República con la IPS y las aseguradoras, de no ser así, acepto todas las consecuencias legales que produzca esta situación.")}).newRow(3).add(new ComponentBuilder[]{DynamicReports.cmp.text("\n\n" + rsper.getString("Administrador")).setPositionType(ComponentPositionType.FLOAT), DynamicReports.cmp.image(this.xmt.getDirectorioExportacion() + "Firmas" + this.xmt.getBarra() + rsper.getString("UrlFA")).setMinDimension(70, 35)}).newRow(3).add(new ComponentBuilder[]{DynamicReports.cmp.horizontalList().setBaseStyle(DynamicReports.stl.style().setTopBorder(DynamicReports.stl.pen1Point()).setLeftPadding(230)), emptyCell(8)}).add(new ComponentBuilder[]{DynamicReports.cmp.horizontalList().setBaseStyle(DynamicReports.stl.style().setTopBorder(DynamicReports.stl.pen1Point()).setLeftPadding(230))}).newRow().add(new ComponentBuilder[]{DynamicReports.cmp.text("NOMBRE"), emptyCell(2), DynamicReports.cmp.text("FIRMA DEL REPRESENTANTE LEGAL, GERENTE O SU DELEGADO")}), DynamicReports.cmp.verticalGap(3)});
            try {
                report.toPdf(pdfExporter);
            } catch (DRException e) {
                Logger.getLogger(Furips_PB_Glosa.class.getName()).log(Level.SEVERE, (String) null, e);
            }
        } catch (SQLException ex) {
            Logger.getLogger(Furips_PB_Glosa.class.getName()).log(Level.SEVERE, (String) null, (Throwable) ex);
        }
        return report;
    }
