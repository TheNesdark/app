...
/* JADX INFO: loaded from: GenomaP.jar:com/genoma/plus/controller/parametrizacion/JIFTipoDieta.class */
public class JIFTipoDieta extends JInternalFrame {
    private DefaultTableModel xmodelo;
    private Object[] xdatos;
    private TipoDietaDTO xTipoDietaDTO;
    ...
    private void springStart() {
        new ClassPathXmlApplicationContext(new String[]{"/recursos/applicationContext.xml"});
    }
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("TIPO DE DIETA");
        setFrameIcon(new ImageIcon(getClass().getResource("/Imagenes/genomap.png")));
        setName("jifTipoDieta");
        try {
            setSelected(true);
        ...
        JPIDatosLayout.setHorizontalGroup(JPIDatosLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(JPIDatosLayout.createSequentialGroup().addContainerGap().addComponent(this.JTFNombre, -2, 424, -2).addGap(28, 28, 28).addComponent(this.JCKBEstado).addContainerGap(-1, 32767)));
        JPIDatosLayout.setVerticalGroup(JPIDatosLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(JPIDatosLayout.createSequentialGroup().addGroup(JPIDatosLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(this.JTFNombre, -2, 47, -2).addComponent(this.JCKBEstado)).addContainerGap(-1, 32767)));
        this.JSPDetalle.setBorder(BorderFactory.createTitledBorder((Border) null, "DETALLE", 2, 0, new Font("Arial", 1, 14), Color.red));
        this.JTDetalle.setFont(new Font("Arial", 1, 12));
        this.JTDetalle.setModel(new DefaultTableModel((Object[][]) new Object[0], new String[0]));
        this.JTDetalle.setRowHeight(25);
        this.JTDetalle.addMouseListener(new MouseAdapter() { // from class: com.genoma.plus.controller.parametrizacion.JIFTipoDieta.1
            public void mouseClicked(MouseEvent evt) {
                JIFTipoDieta.this.JTDetalleMouseClicked(evt);
            }
        });
    ...
    public void mGrabar() {
        if (!this.JTFNombre.getText().isEmpty()) {
            this.xTipoDietaDTO = new TipoDietaDTO();
            if (this.JTDetalle.getSelectedRow() >= 0) {
                this.xTipoDietaDTO.setId(this.id);
            }
            this.xTipoDietaDTO.setNombre(this.JTFNombre.getText().trim());
            this.xTipoDietaDTO.setEstado(Boolean.valueOf(this.JCKBEstado.isSelected()));
            this.xTipoDietaDTO.setFecha(this.xmetodos.getFechaActual());
            if (this.JCKBEstado.isSelected()) {
            }
            this.xITipoDietaDAO.grabar(this.xTipoDietaDTO);
            mNuevo();
            return;
        }
    ...
    private void mCrearModeloDatos() {
        this.xmodelo = new DefaultTableModel(new Object[0], new String[]{"id", "nombre", "estado", "fecha"}) { // from class: com.genoma.plus.controller.parametrizacion.JIFTipoDieta.2
    ...
    private void mCargarDatosTabla() {
        List<TipoDietaDTO> tabla = this.xITipoDietaDAO.consultarTipoDieta();
        mCrearModeloDatos();
