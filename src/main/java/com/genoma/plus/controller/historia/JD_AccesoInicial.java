...
/* JADX INFO: loaded from: GenomaP.jar:com/genoma/plus/controller/historia/JD_AccesoInicial.class */
public class JD_AccesoInicial extends JDialog {
    ...
    public JD_AccesoInicial(Frame parent, boolean modal, Principal frmPrincipal, Integer modulo) {
        super(parent, modal);
    ...
    private void initComponents() {
        ...
        this.JCB_Especialidad.addItemListener(new ItemListener() { // from class: com.genoma.plus.controller.historia.JD_AccesoInicial.1
            public void itemStateChanged(ItemEvent evt) {
                JD_AccesoInicial.this.JCB_EspecialidadItemStateChanged(evt);
            }
        });
        ...
        this.JBT_Ok.addActionListener(new ActionListener() { // from class: com.genoma.plus.controller.historia.JD_AccesoInicial.2
            public void actionPerformed(ActionEvent evt) {
                JD_AccesoInicial.this.JBT_OkActionPerformed(evt);
            }
        });
        ...
        this.JBT_Salir.addActionListener(new ActionListener() { // from class: com.genoma.plus.controller.historia.JD_AccesoInicial.3
            public void actionPerformed(ActionEvent evt) {
                JD_AccesoInicial.this.JBT_SalirActionPerformed(evt);
            }
        });
        ...
        layout.setHorizontalGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(layout.createSequentialGroup().addContainerGap().addGroup(layout.createParallelGroup(GroupLayout.Alignment.TRAILING, false).addComponent(this.JLB_Titulo, -1, 534, 32767).addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING, false).addComponent(this.JPI_Datos, GroupLayout.Alignment.TRAILING, -1, -1, 32767).addGroup(layout.createSequentialGroup().addComponent(this.JBT_Ok, -2, 200, -2).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED, 134, 32767).addComponent(this.JBT_Salir, -2, 200, -2)))).addContainerGap(16, 32767)));
        ...
        if (this.JCB_Especialidad.getSelectedIndex() != -1) {
            if (this.idServicio != null) {
                if (this.JCB_Servicio.getSelectedIndex() != -1) {
                    System.out.println("ENtrooo  aca");
    ...
    private void LlenarComboServicio() {
        List<Object[]> listServicio;
        this.JCB_Servicio.removeAllItems();
        if (this.modulo.intValue() == 0) {
            listServicio = this.informacionVariaUrgenciasHospitalizacionDAO.listadoServiciosEspecialidad(Long.valueOf(this.idEspecialidad[this.JCB_Especialidad.getSelectedIndex()]), 0);
        } else {
            listServicio = this.informacionVariaUrgenciasHospitalizacionDAO.listadoServiciosEspecialidadFiltroCirugia(Long.valueOf(this.idEspecialidad[this.JCB_Especialidad.getSelectedIndex()]));
        }
