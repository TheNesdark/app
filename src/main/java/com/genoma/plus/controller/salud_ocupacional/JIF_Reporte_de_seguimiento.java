...
import com.genoma.plus.dto.salud_ocupacional.Reporte_de_seguimientoDTO;
...
/* JADX INFO: loaded from: GenomaP.jar:com/genoma/plus/controller/salud_ocupacional/JIF_Reporte_de_seguimiento.class */
public class JIF_Reporte_de_seguimiento extends JInternalFrame {
    private DefaultTableModel xModelo;
    private Object[] xDato;
    private Reporte_de_seguimientoDAO xReporte_de_seguimientoDAO;
    private Reporte_de_seguimientoDTO dto;
    ...
    private List<Object[]> listCombo = null;

    public JIF_Reporte_de_seguimiento() {
        initComponents();
        springStart();
        mNuevo();
    }
    ...
    private void springStart() {
        this.xReporte_de_seguimientoDAO = (Reporte_de_seguimientoDAO) new ClassPathXmlApplicationContext(new String[]{"/recursos/applicationContext.xml"}).getBean("reporte_de_seguimientoDAOImpl");
    }

    public void mNuevo() {
        mCrearModeloReporte_de_seguimiento();
        mCargarCombo();
    ...
    private void mCargarCombo() {
        this.listCombo = null;
        this.listCombo = this.xReporte_de_seguimientoDAO.CargarCombo();
        for (int i = 0; i < this.listCombo.size(); i++) {
            this.JCBPeriodo.addItem(this.listCombo.get(i)[1].toString());
        }
        this.JCBPeriodo.setSelectedIndex(-1);
    }

    private void mCrearModeloReporte_de_seguimiento() {
        this.xModelo = new DefaultTableModel((Object[][]) null, new String[]{"Id", "Cedula", "NUsuario", "Nbre", "UnidadN", "Nbre", "Nbre", "FechaPA", "Alterado", "ConceptoI", "Id_Atencion", "Id_Persona", "EstadoS", "RT", "HC", "Edad", "FEntrada", "tiempo", "NSexo", "seguimiento", "observacion", "NDiasS", " FechaUS", "Fecha_Cierre", " FC", " vigilancia", "FP", "Canti"}) { // from class: com.genoma.plus.controller.salud_ocupacional.JIF_Reporte_de_seguimiento.1
    ...
    private void mCargarDatosTablaReporte_de_seguimiento() {
        mCrearModeloReporte_de_seguimiento();
        String periodo = "";
        for (int i = 0; i < this.listCombo.size(); i++) {
            if (this.JCBPeriodo.getSelectedIndex() == i) {
                periodo = this.listCombo.get(i)[0].toString();
            }
        }
        List<Object[]> list = this.xReporte_de_seguimientoDAO.Reporte_seguimientoDAO(periodo);
    ...
    public void mBuscar() {
        if (this.JCBPeriodo.getSelectedIndex() != -1) {
            mCargarDatosTablaReporte_de_seguimiento();
        } else {
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("Examenes periodicos seguimiento detalles");
        setFrameIcon(new ImageIcon(getClass().getResource("/Imagenes/genomap.png")));
        setName("jif_Reporte_de_seguimiento");
        ...
        JPDatosLayout.setHorizontalGroup(JPDatosLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(this.JCBPeriodo, 0, 302, 32767));
        JPDatosLayout.setVerticalGroup(JPDatosLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(this.JCBPeriodo, -1, 63, 32767));
        ...
        this.JTFRuta.addMouseListener(new MouseAdapter() { // from class: com.genoma.plus.controller.salud_ocupacional.JIF_Reporte_de_seguimiento.2
            public void mouseClicked(MouseEvent evt) {
                JIF_Reporte_de_seguimiento.this.JTFRutaMouseClicked(evt);
            }
        });
        this.JTFRuta.addActionListener(new ActionListener() { // from class: com.genoma.plus.controller.salud_ocupacional.JIF_Reporte_de_seguimiento.3
            public void actionPerformed(ActionEvent evt) {
                JIF_Reporte_de_seguimiento.this.JTFRutaActionPerformed(evt);
            }
        });
        ...
        this.JBTExportar.addActionListener(new ActionListener() { // from class: com.genoma.plus.controller.salud_ocupacional.JIF_Reporte_de_seguimiento.4
            public void actionPerformed(ActionEvent evt) {
                JIF_Reporte_de_seguimiento.this.JBTExportarActionPerformed(evt);
            }
        });
