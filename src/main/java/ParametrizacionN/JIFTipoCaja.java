...
/* JADX INFO: loaded from: GenomaP.jar:ParametrizacionN/JIFTipoCaja.class */
public class JIFTipoCaja extends JInternalFrame {
    ...
    private void initComponents() {
        ...
        this.JCHEstado.addActionListener(new ActionListener() { // from class: ParametrizacionN.JIFTipoCaja.1
        ...
        JPIDatosLayout.setHorizontalGroup(JPIDatosLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(JPIDatosLayout.createSequentialGroup().addContainerGap().addGroup(JPIDatosLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(JPIDatosLayout.createSequentialGroup().addComponent(this.JTFFMonto, -2, 130, -2).addGap(18, 18, 18).addComponent(this.JTFFSaldo, -2, 130, -2).addGap(18, 18, 18).addComponent(this.JSPPorcentaje, -2, 57, -2).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED, 65, 32767).addComponent(this.JCHEstado)).addComponent(this.JTFNombre, GroupLayout.Alignment.TRAILING, -1, 483, 32767)).addContainerGap()));
        JPIDatosLayout.setVerticalGroup(JPIDatosLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(JPIDatosLayout.createSequentialGroup().addContainerGap().addComponent(this.JTFNombre, -2, -1, -2).addGap(12, 12, 12).addGroup(JPIDatosLayout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(this.JTFFMonto, -1, 50, 32767).addComponent(this.JTFFSaldo, -1, 50, 32767).addComponent(this.JCHEstado).addComponent(this.JSPPorcentaje))));
        ...
        this.JTDetalle.addMouseListener(new MouseAdapter() { // from class: ParametrizacionN.JIFTipoCaja.2
        ...
        if (!this.JTFNombre.getText().isEmpty()) {
            ...
            if (n == 0) {
                if (Principal.txtNo.getText().isEmpty()) {
                    sql = " Insert into cc_tipo_caja(Nbre, Monto, Saldo, PorcentajeR, Estado, Fecha, UsuarioS)  values ('" + this.JTFNombre.getText().toUpperCase() + "','" + this.JTFFMonto.getValue() + "','" + this.JTFFSaldo.getValue() + "','" + this.JSPPorcentaje.getValue() + "','" + this.xestado + "','" + this.xmetodos.formatoAMDH24.format(this.xmetodos.getFechaActual()) + "','" + Principal.usuarioSistemaDTO.getLogin() + "')";
                } else {
                    sql = " update cc_tipo_caja set Nbre='" + this.JTFNombre.getText().toUpperCase() + "',Monto='" + this.JTFFMonto.getValue() + "', Saldo='" + this.JTFFSaldo.getValue() + "', PorcentajeR='" + this.JSPPorcentaje.getValue() + "',Estado='" + this.xestado + "', Fecha='" + this.xmetodos.formatoAMDH24.format(this.xmetodos.getFechaActual()) + "', UsuarioS='" + Principal.usuarioSistemaDTO.getLogin() + "' Where Id='" + Principal.txtNo.getText() + "'";
                }
    ...
    private void mCrearModeloDatos() {
        this.xmodelo = new DefaultTableModel(new Object[0], new String[]{"Id", "Nombre", "Monto", "Saldo", "%", "Estado"}) { // from class: ParametrizacionN.JIFTipoCaja.3
            Class[] types = {Long.class, String.class, Double.class, Double.class, Double.class, Boolean.class};
    ...
    private void mCargarDatosTabla() {
        try {
            mCrearModeloDatos();
            ResultSet xrs = this.xconsulta.traerRs("SELECT Id, Nbre, Monto, Saldo, PorcentajeR, Estado FROM cc_tipo_caja ORDER BY Nbre ASC");
            if (xrs.next()) {
                xrs.beforeFirst();
                int n = 0;
                while (xrs.next()) {
