...
/* JADX INFO: loaded from: GenomaP.jar:Historia/ReporteRemision.class */
public class ReporteRemision {
    ...
    private String xid_remision;
    private Metodos xmt = new Metodos();
    private ConsultasMySQL xct = new ConsultasMySQL();
    private String xpersona = "";
    private Connection connection = this.xct.establecerConexionBd();

    public ReporteRemision(String xidAtencion, String xid_remision) {
        this.xidAtencion = "";
        this.xid_remision = "";
        this.xidAtencion = xidAtencion;
        this.xid_remision = xid_remision;
    }

    public JasperReportBuilder build() {
        this.xmt.mEliminarArchivo(new File(this.xmt.getRutaRep() + "FReferencia_" + this.xid_remision + ".pdf"));
        JasperPdfExporterBuilder pdfExporter = DynamicReports.export.pdfExporter(this.xmt.getRutaRep() + "FReferencia_" + this.xid_remision + ".pdf");
    ...
    private HorizontalListBuilder Firma(String firma) {
        ...
        if (!RutaFirma.isEmpty()) {
            list = DynamicReports.cmp.horizontalList().add(new ComponentBuilder[]{DynamicReports.cmp.image(RutaFirma + firma).setPositionType(ComponentPositionType.FLOAT).setMinDimension(80, 50)});
        } else {
            list = DynamicReports.cmp.horizontalList().add(new ComponentBuilder[]{DynamicReports.cmp.image(this.xmt.getSO() + this.xmt.getBarra() + "Firmas" + this.xmt.getBarra() + firma).setPositionType(ComponentPositionType.FLOAT).setMinDimension(80, 50)});
        }
        return list;
    }
}
