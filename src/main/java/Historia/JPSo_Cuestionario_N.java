...
/* JADX INFO: loaded from: GenomaP.jar:Historia/JPSo_Cuestionario_N.class */
public class JPSo_Cuestionario_N extends JPanel {
    public int xtipo;
    ...
    public JPSo_Cuestionario_N(String xnombre, int xtipo, Integer idClasificacion) {
        this.xtipo = 1;
        initComponents();
        this.JTPDatos.remove(this.JPIResultado);
        springStart();
    ...
    private JTableComboBox mLlenaComboModelo1(int xid, int xFila) {
        try {
            ...
            Logger.getLogger(JPSo_Cuestionario_N.class.getName()).log(Level.SEVERE, (String) null, e);
        }
    ...
    private void mCrearModeloTablaS() {
        ...
        this.xmodelocombo = new DefaultTableModel(new Object[0], columnNames) { // from class: Historia.JPSo_Cuestionario_N.1
        ...
        this.tabla_combo = new JTable(this.xmodelocombo) { // from class: Historia.JPSo_Cuestionario_N.2
            public TableCellEditor getCellEditor(int row, int column) {
                int modelColumn = convertColumnIndexToModel(column);
                if (modelColumn == 4 && row < JPSo_Cuestionario_N.this.tabla_combo.getRowCount()) {
                    return (TableCellEditor) JPSo_Cuestionario_N.this.editors.get(row);
                }
    ...
    private void initComponents() {
        ...
        this.JTCategoria.addPropertyChangeListener(new PropertyChangeListener() { // from class: Historia.JPSo_Cuestionario_N.3
            @Override // java.beans.PropertyChangeListener
            public void propertyChange(PropertyChangeEvent evt) {
                JPSo_Cuestionario_N.this.JTCategoriaPropertyChange(evt);
            }
        });
        ...
        JPIPreguntasLayout.setHorizontalGroup(JPIPreguntasLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(JPIPreguntasLayout.createSequentialGroup().addContainerGap().addGroup(JPIPreguntasLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(this.JSPDetalleP).addGroup(JPIPreguntasLayout.createSequentialGroup().addComponent(this.JSPObservacionG, -2, 344, -2).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(this.JSPCategoria, -1, 438, 32767))).addContainerGap()));
        ...
        this.JTDetalleCR.addMouseListener(new MouseAdapter() { // from class: Historia.JPSo_Cuestionario_N.4
            public void mouseClicked(MouseEvent evt) {
                JPSo_Cuestionario_N.this.JTDetalleCRMouseClicked(evt);
            }
        });
        this.JTDetalleCR.addPropertyChangeListener(new PropertyChangeListener() { // from class: Historia.JPSo_Cuestionario_N.5
            @Override // java.beans.PropertyChangeListener
            public void propertyChange(PropertyChangeEvent evt) {
                JPSo_Cuestionario_N.this.JTDetalleCRPropertyChange(evt);
            }
        });
        this.JTDetalleCR.addKeyListener(new KeyAdapter() { // from class: Historia.JPSo_Cuestionario_N.6
            public void keyPressed(KeyEvent evt) {
                JPSo_Cuestionario_N.this.JTDetalleCRKeyPressed(evt);
            }
        });
        ...
        JPHistoricoLayout.setHorizontalGroup(JPHistoricoLayout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(GroupLayout.Alignment.TRAILING, JPHistoricoLayout.createSequentialGroup().addContainerGap().addComponent(this.jScrollPane1, -1, 788, 32767).addContainerGap()));
        ...
        this.JBTGrabar.addActionListener(new ActionListener() { // from class: Historia.JPSo_Cuestionario_N.7
            public void actionPerformed(ActionEvent evt) {
                JPSo_Cuestionario_N.this.JBTGrabarActionPerformed(evt);
            }
        });
        ...
        setLayout(layout);
        layout.setHorizontalGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(layout.createSequentialGroup().addContainerGap().addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(this.JLB_Encabezado, -1, -1, 32767).addGroup(layout.createSequentialGroup().addComponent(this.JBTGrabar, -2, 536, -2).addGap(4, 4, 4).addComponent(this.JLB_Resultado, -2, 116, -2).addPreferredGap(LayoutStyle.ComponentPlacement.UNRELATED).addComponent(this.JLBNRegistro, -2, 120, -2).addGap(0, 0, 32767))).addContainerGap()).addComponent(this.JTPDatos));
    ...
    private void setEventoMouseClicked(JTable tbl) {
        tbl.addMouseListener(new MouseAdapter() { // from class: Historia.JPSo_Cuestionario_N.8
            public void mouseClicked(MouseEvent e) {
                JPSo_Cuestionario_N.this.tabla_comboMouseClicked(e);
            }
        });
    }
    ...
    private void mCrearModeloTablaR() {
        this.xmodeloR = new DefaultTableModel(new Object[0], new String[]{"Id", "Nombre", "VMinimo", "VMaximo", "Seleccion", "Referencia"}) { // from class: Historia.JPSo_Cuestionario_N.9
    ...
    private void mCrearModeloTablaHistorico() {
        this.xModeloHistorico = new DefaultTableModel(new Object[0], new String[]{"Fecha", "Pregunta", "Respuesta", "Observación"}) { // from class: Historia.JPSo_Cuestionario_N.10
    ...
    private void mCargarResultado() {
        ...
        if (!valor.isEmpty()) {
            ...
            try {
                ...
                Logger.getLogger(JPSo_Cuestionario_N.class.getName()).log(Level.SEVERE, (String) null, (Throwable) ex);
                return;
            }
        }
    ...
    private void mCargarDatosTabla(int xforma) {
        ...
        if (list.size() > 0) {
            ...
            JTableComboBox[] itemsCombo = new JTableComboBox[list.size()];
            ...
            for (int x = 0; x < list.size(); x++) {
                ...
                if (xforma == 0) {
                    itemsCombo[x] = mLlenaComboModelo1(Integer.valueOf(list.get(x)[0].toString()).intValue(), x);
                    itemsCombo[x].setSelectedIndex(-1);
                    DefaultCellEditor defaultCellEditor = new DefaultCellEditor(itemsCombo[x]);
                    this.editors.add(defaultCellEditor);
                    this.xmodelocombo.setValueAt(defaultCellEditor.getCellEditorValue(), x, 4);
                    this.xmodelocombo.setValueAt(this.JLBNRegistro.getText(), x, 5);
                    this.xmodelocombo.setValueAt(0, x, 6);
                    this.xmodelocombo.setValueAt(Double.valueOf(itemsCombo[x].getPopupTable().getValueAt(0, 2).toString()), x, 7);
                    this.xmodelocombo.setValueAt(13, x, 8);
                } else {
                    itemsCombo[x] = mLlenaComboModelo1(Integer.valueOf(list.get(x)[0].toString()).intValue(), x);
                    this.editors.add(new DefaultCellEditor(itemsCombo[x]));
                ...
                this.xcomboBox3[x].addItemListener(new ItemListener() { // from class: Historia.JPSo_Cuestionario_N.11
                    public void itemStateChanged(ItemEvent e) {
                        JPSo_Cuestionario_N.this.mCambiaElementoCombo();
                    }
                });
            }
            mSumatoria();
    ...
    /* JADX INFO: loaded from: GenomaP.jar:Historia/JPSo_Cuestionario_N$MiRender.class */
    ...
    private void mCrearTablaSubCategoria() {
        this.xmodeloSubC = new DefaultTableModel((Object[][]) null, new String[]{"Id", "Nombre", "Observación", "Aplica?"}) { // from class: Historia.JPSo_Cuestionario_N.12
